...
function ContinualResolving:_sample_bet(node, state)
  ...
  local hand_variance = arguments.Tensor(actions_count)
  ...
  for i = 1, actions_count do
    ...
    --gets the variance of cfv value if this action is taken
    local action_variance = self.resolving:get_action_cfv(action_bet)
    
    hand_variance[i] = torch.var(action_variance)
    hand_strategy[i] = action_strategy[self.hand_id]

  end

  print("hand variance", hand_variance)
  print("hand strategy", hand_strategy)
  ...
  --MODIFICATION FOR MILD ADAPTIVE ROCKS

  --take all actions with prob > .15 (if exist), choose one with lowest variance with flip of coin
  --see which are greater than 0, get the one with lowest variance
  local greater_than_zero = {}
  count, key, min = 0,0,100000000
  for i = 1, actions_count do
    if hand_strategy[i] > 0.15 then 
      greater_than_zero[i] = hand_variance[i]
      count = count + 1
      if hand_variance[i] < min then 
        key = i
        min = hand_variance[i]
      end
    end 
  ...
  --only in 50% of cases do the rocks strategy
  if count > 0 and randomnum > 0.5 then
    print("playing it save with the lowest variance action", possible_bets[key])
    sampled_bet = possible_bets[key]
  else
    print("doing original move, being risky", sampled_bet)
  end
